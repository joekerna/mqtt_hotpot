...
// Filter pump
#define FILTER_RELAY D5
unsigned long filterLastOnMillis = 0;
...
void mqtt_callback(char *topic, byte *payload, unsigned int length) {
  ...
  // }
  if (topic, String(String(mqtt_topic) + "/temp_threshold"))
  {
...
// -------------------------------------------------------------------------
  ...
  createNewSensor("Vorlauf",   "temperature", temperature_state_topic, "unique_id_vor",   "hotpot_temperature", device_name, manufacturer, model, model_id, "sensor", "{{ value_json.vorlauf }}");

  // Create Ruecklauf Sensor
  createNewSensor("Rücklauf",  "temperature", temperature_state_topic, "unique_id_rueck", "hotpot_temperature", device_name, manufacturer, model, model_id, "sensor", "{{ value_json.ruecklauf }}");

  // Create Difference Sensor
  createNewSensor("Differenz", "temperature", temperature_state_topic, "unique_id_diff",  "hotpot_temperature", device_name, manufacturer, model, model_id, "sensor", "{{ value_json.difference }}");

  // Create Fire state Sensor
  createNewSensor("Feuer",     "heat",        fire_state_topic,        "unique_id_fire_state", "hotpot_temperature", device_name, manufacturer, model, model_id, "binary_sensor");

  // Create Freeze state Sensor
  createNewSensor("Frost",     "cold",        freeze_state_topic,      "unique_id_freeze_state", "hotpot_temperature", device_name, manufacturer, model, model_id, "binary_sensor");
    ...
    // Check MQTT connection
     ...
     if ( (fabs(vor_change) >= temperature_change_threshold) || (fabs(rueck_change) >= temperature_change_threshold) )
     {
        ...
        if (temp_difference > fire_threshold) {
          client.publish(fire_state_topic, "ON");
        } else {
          client.publish(fire_state_topic, "OFF");
        }

        // Publish Freeze state
        if ((temp_rueck < freeze_threshold) | (temp_vor < freeze_threshold)) {
          client.publish(freeze_state_topic, "ON");
        } else {
          client.publish(freeze_state_topic, "OFF");
        }
     }
  }
